...
const Login = () => {
  ...
  const onFinish = async (values) => {
    ...
    if(isRegistering){
      // 注册逻辑
      try {
        const response = await fetch('http://43.143.114.225:8000/register/', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded', // 注意这里修改为 JSON 格式
          },
      ...
      // 登录逻辑
      try {
        const response = await fetch('http://43.143.114.225:8000/login/', {
          method: 'POST',
          headers: {
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
      ...
      <Card style={{ width: isMobile ? '90%' : 300 }}>
        <h2 style={{ textAlign: 'center' }}>{isRegistering ? '注册' : '登录书法百宝箱'}</h2> {/* 根据状态显示标题 */}
        <Form
          name="login-form"
